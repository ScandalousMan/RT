...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/08/29 14:46:17 by itsalex           #+#    #+#             */
/*   Updated: 2018/08/29 14:46:18 by itsalex          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			snell_descartes(double n1, double n2, t_path *path1,
				t_path *path2)
{
	if (n1 == n2 || ft_absdbl(scalar_product(path1->v, path1->n) + 1.0)
		< EPSILON)
	{
	...
	if (scalar_product(path1->v, path1->n) == 0.0 ||
		ft_absdbl(n1 / n2 * sin(acos(scalar_product(path1->v, path1->n))))
			>= 1.0)
		return (0);
	vec_multiply(1.0 / scalar_product(path1->v, path1->n), path1->v,
		path2->v);
	vec_soustraction(path1->n, path2->v, path2->v);
	vec_to_unit_norm(path2->v);
	vec_multiply(tan(asin(n1 / n2 * sin(acos(scalar_product(path1->v,
		path1->n))))), path2->v, path2->v);
