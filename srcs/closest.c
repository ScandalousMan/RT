...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/10/06 11:04:08 by vacrozet          #+#    #+#             */
/*   Updated: 2018/10/06 11:04:11 by vacrozet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	t_path *path)
{
	...
	while (objs)
	{
		if (!param->is_for_light || objs != path->current_object)
		{
			...
			if (param->tmp_d > 0.0 &&
				(param->is_for_light || (param->obj_d < 0.0 || param->tmp_d <
					param->obj_d)))
			{
				...
				while (limits && param->tmp_d > 0 && param->is_cut)
				{
					...
					{
						param->tmp_d = plane_distance(from, to,
							limits->plane.n, limits->plane.ref);
						vec_multiply(param->tmp_d + param->epsilon,
							to, path->valid_x);
						pt_translated(from, path->valid_x, path->valid_x);
						if (param->tmp_d > 0 && is_inside_object(path->valid_x,
							objs, limits))
						{
							param->is_cut = 0;
				...
				if (param->is_cut != 1 && param->tmp_d > 0.0)
				{
					...
					{
						vec_multiply(param->obj_d - param->epsilon, to,
							path->x);
						pt_translated(from, path->x, path->x);
						vec_copy(path->valid_n, path->n);
					}
					else if (param->tmp_light->type != RTSPOT ||
						param->obj_d * param->obj_d < pt_dist_root(from,
							param->tmp_light->src))
					{
						param->tmp_light->tmp_col = rgb_ratio(
							color_absorber(objs->col,
							param->tmp_light->tmp_col), objs->transparency);
					}
				}
			}
		}
...
void		object_color_changer(t_object *object, t_param *param)
{
	...
	{
		t = wood_ratio(object->uv_map[0] * 128.0, object->uv_map[1] *
			128.0, TURB_SIZE, param);
	...
	t_path *path)
{
	...
	{
		rotation_matrice(0, 0, 45 *
			sin(object->uv_map[0] * 2.0 * M_PI * 20), param);
