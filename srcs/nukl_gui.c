...
void	nukl_gui(t_param *param)
{
	...
	{
		nk_layout_row_dynamic(param->graph->ctx, 30, 1);
		nk_label(param->graph->ctx, "Global settings", NK_TEXT_CENTERED);
		...
		{
			nk_layout_row_push(param->graph->ctx, 0.50f);
			if (nk_button_label(param->graph->ctx, "REFRESH"))
				mprintf(1, "Refresh engaged\n");
			nk_layout_row_push(param->graph->ctx, 0.50f);
			if (nk_button_label(param->graph->ctx, "RESET"))
				mprintf(1, "RESET engaged\n");
		}
	}
