...
/*   Updated: 2018/09/09 17:58:33 by vacrozet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
					...
					WINDOW_GUI_HEIGHT), NK_WINDOW_SCALE_LEFT))
	{
		nk_layout_row_begin(param->graph->ctx, NK_STATIC, 30, 3);
		{
			...
			nk_slider_int(param->graph->ctx, MIN_ANTI_ALIASING,
					&param->macro.anti_aliasing, MAX_ANTI_ALIASING,
					STEP_ANTI_ALIASING);
			nk_layout_row_push(param->graph->ctx, 100);
		...
		nk_layout_row_begin(param->graph->ctx, NK_STATIC, 30, 3);
		{
			...
			nk_layout_row_push(param->graph->ctx, 200);
			nk_slider_int(param->graph->ctx, MIN_RECURSION,
					&param->macro.recursion, MAX_RECURSION, STEP_RECURSION);
			nk_layout_row_push(param->graph->ctx, 100);
		...
		nk_layout_row_begin(param->graph->ctx, NK_STATIC, 30, 3);
		{
			...
			nk_slider_int(param->graph->ctx, MIN_CARTOON_FACTOR,
					&param->macro.cartoon_factor, MAX_CARTOON_FACTOR,
					STEP_CARTOON_FACTOR);
			nk_layout_row_push(param->graph->ctx, 100);
		...
		nk_layout_row_begin(param->graph->ctx, NK_STATIC, 30, 3);
		{
			...
			nk_slider_int(param->graph->ctx, MIN_BLUR_RADIUS,
					&param->macro.blur_radius, MAX_BLUR_RADIUS,
					STEP_BLUR_RADIUS);
			nk_layout_row_push(param->graph->ctx, 100);
		...
		nk_layout_row_begin(param->graph->ctx, NK_STATIC, 30, 3);
		{
			...
			nk_slider_int(param->graph->ctx, MIN_ROTATION_ANGLE,
					&param->macro.rotation_angle, MAX_ROTATION_ANGLE,
					STEP_ROTATION_ANGLE);
			nk_layout_row_push(param->graph->ctx, 100);
		...
		nk_layout_row_begin(param->graph->ctx, NK_STATIC, 30, 3);
		{
			...
			nk_slider_int(param->graph->ctx, MIN_SPECULAR_EXP,
					&param->macro.specular_exp, MAX_SPECULAR_EXP,
					STEP_SPECULAR_EXP);
			nk_layout_row_push(param->graph->ctx, 100);
		...
		nk_layout_row_begin(param->graph->ctx, NK_STATIC, 30, 3);
		{
			...
			nk_slider_float(param->graph->ctx, MIN_K_AMBIENCE,
					&param->macro.k_ambience, MAX_K_AMBIENCE,
					STEP_K_AMBIENCE);
			nk_layout_row_push(param->graph->ctx, 100);
...
=======
			param->macro.filter = nk_combo(param->graph->ctx, filter, 5,
					param->macro.filter, 20, sizeButton);
>>>>>>> norminette toute simple
		}
