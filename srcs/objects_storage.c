...
/*   Updated: 2018/10/06 10:49:07 by vacrozet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			*fill_customobject(t_jobject *jobj, t_param *param)
{
	...
	if (!(obj = get_custom_ptr(tmp->value, param->customs)))
		error(0, 0, "Can't find one custom object");
	fill_vector(&(obj->org), (t_jarray*)(get_jobject(jobj, "center")->value));
	fill_vector(&tr, (t_jarray*)(get_jobject(jobj, "translation")->value));
	obj->org[0] += tr[0];
	obj->org[1] += tr[1];
...
{
	const t_object_def obj_def = get_object_def_by_name(get_jobject(jobj,
		RT_OBJECT_TYPE)->value);
...
t_custom_obj	*get_custom_obj(t_jarray *array, t_param *param, t_custom_obj
	*custom_obj)
{
...
int				camera_storage(t_jobject *obj, t_param *param)
{
	...
	fill_vector(&(param->align),
		(t_jarray*)(get_jobject(tmp, "align")->value));
	return (1);
}
...
{
	param->customs = get_custom_name(get_jobject(obj, CUSTOMS_KEY)->value, 1,
		param);
	param->customs = get_custom(get_jobject(obj, CUSTOMS_KEY)->value, 1,
		param, param->customs);
